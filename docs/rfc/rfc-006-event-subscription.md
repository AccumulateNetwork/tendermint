...
- **Event delivery is not valid JSON-RPC.** When a client issues a `subscribe`
  ...
  (`{}`). After that, each matching event is delivered as a separate [response
  object][json-response] with the same ID as the initial request.
...
- **Subscriptions cannot survive disconnection.** When the connection is
  interrupted, the subscription is entirely lost.

  This matters because a client whose connection is dropped gets no useful
  error feedback, just a closed connection.  Should they try again?  Is the
  node overloaded?  Was the client too slow?  Did the caller forget to respond
  to pings? Debugging these kinds of failures is unnecessarily painful.

  Websockets compound this, because websocket connections time out if no
  traffic is seen for a while, and keeping it alive requires active cooperation
  between the client and server.  With a plain TCP socket, liveness is handled
  transparently by the keepalive mechanism, but on a websocket, one side has to
  occasionally send a PING (if the connection is otherwise idle).  The other
  side must return a matching PONG in time, or the connection is dropped.
  Apart from being tedious, this is highly susceptible to CPU load.
...
- **Mismatched load profiles.** Most of the RPC service is mainly important for
  ...
  aside the non-standard protocol implementation mentioned above, the time and
  memory cost of encoding event data matters for the stability of the node when
  there can be potentially hundreds of subscribers. Moreover, a subscription is
...
2. **Use a different protocol for event subscriptions.** There are different
   ways we could approach this, depending how much we're willing to shake up
   the current API. Here are sketches of a few options:

   - Keep the websocket, but rework the API to be JSON-RPC compliant.  This is
     less up-front work for existing clients, but retains a lot of the existing
     implementation complexity, and may prevent us from making more serious
     performance improvements later.

   - Keep HTTP, but rework the API to use a more conventional request/response
     pattern instead of streaming. This is a little more up-front work for
     existing clients, but leverages better library support for clients not
     written in Go. The protocol might be more chatty, but we could hedge that
     with batching. This has a nice incidental benefit that it's easy to debug
     from the command-line like any of the other RPC methods.
